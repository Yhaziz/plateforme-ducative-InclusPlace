...
// Query for "new" events and events with end_date equal to today and category_id = 1
$query = "SELECT id, date_creation, title, start_time, end_time, start_date, end_date, location, image_name, end_date 
          FROM events 
          WHERE (end_date > '$today' OR (end_date = '$today' AND date_creation < '$today'))
          AND category_id = 1"; // Add the condition for category_id
