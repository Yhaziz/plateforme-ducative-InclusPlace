...
$sql1="SELECT * FROM subject_routing WHERE teacher_id='$teacher_id' AND classroom_id='$classroom_id'";
$result1=mysqli_query($conn,$sql1);

while($row1=mysqli_fetch_assoc($result1)){
	$sr_id=$row1['id'];
	
	$sql2="select student.surname as std_name,student.id as std_id,student.index_number as std_index,subject_routing.subject_id as s_id
...
?>   
          			<tr>
                    	...
                    	<td> 
<?php
$sql3="SELECT * FROM student_exam WHERE index_number='$std_index' AND classroom_id='$classroom_id' AND exam_id='$exam_id' AND year='$year' AND subject_id='$subject_id'";

$result3=mysqli_query($conn,$sql3);
if(mysqli_num_rows($result3) > 0){
	echo'<a href="#" class=" btn btn-success btn-xs" onClick="showModalEditExam('.$exam_id.','.$my_index.','.$std_index.','.$classroom_id.')" >Edit Exam Marks</a>
<a href="#" class=" btn btn-primary btn-xs" onClick="showModalViewExam('.$exam_id.','.$year.','.$std_index.')"  >View Exam Marks</a>';
	
}else{
	echo'<a href="#" class=" btn btn-warning btn-xs" onClick="showModalAddExam('.$exam_id.','.$my_index.','.$std_index.','.$classroom_id.')"  >Add Exam Marks</a>';
}
?>
