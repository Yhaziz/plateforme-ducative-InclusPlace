...
if(mysqli_num_rows($result) > 0){
	while($row=mysqli_fetch_assoc($result)){
		...
		if($friend_type=="Parent"){
			$sql1="SELECT * FROM parent WHERE index_number='$friend_index'";
			$result1=mysqli_query($conn,$sql1);
			$row1=mysqli_fetch_assoc($result1);
			$friend_name=$row1['surname'];
		...
		if($friend_type=="Avs"){
			$sql1="SELECT * FROM avs WHERE index_number='$friend_index'";
			$result1=mysqli_query($conn,$sql1);
			$row1=mysqli_fetch_assoc($result1);
			$friend_name=$row1['surname'];
			$friend_image=$row1['image_name'];
		...
		if($friend_type=="Doctor"){
			$sql1="SELECT * FROM doctor WHERE index_number='$friend_index'";
			$result1=mysqli_query($conn,$sql1);
			$row1=mysqli_fetch_assoc($result1);
			$friend_name=$row1['fname'];
			$friend_image=$row1['image_name'];
		}
...
?>   
                                <tr>
									...
									<a href="#"onClick="friendProfile('<?php echo $friend_type; ?>','<?php echo $friend_index; ?>')"><?php echo $friend_name; ?> </a><br>
                                        ...
                                        <?php 
											if($friend_type == "Student"){
												...
												$sql2="select classroom.name as g_name 
											...
											if($friend_type == "Parent"){
												echo "<small> Parent </small>";
											}

											if($friend_type == "Avs"){
												echo "<small> Avs </small>";
											}
                                    ...
                                    <td>
<!--MSK-00146 --> 
<a href="#" onClick="showModal('<?php echo $my_index; ?>','<?php echo $conversation_id; ?>','<?php echo $friend_index; ?>','<?php echo $my_type; ?>')" class="btn btn-primary btn-xs"  >Send Message</a>
                                    </td>
