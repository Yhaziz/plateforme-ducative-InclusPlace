...
if(isset($_POST["do"])&&($_POST["do"]=="update_doctor")){
	...
	if($email == $email1 && $id == $id1){//MSK-000143-U-1
		...
		if($cin != $cin1 || $fname != $fname1 || $surname != $surname1 ||$Spec != $Spec1 || $address != $address1 || $gender != $gender1 ||$téléphone != $téléphone1 || $name){//MSK-000143-U-2		
			
			if(!$name){//MSK-000143-U-3
				$sql = "update doctor set cin='".$cin."', fname='".$fname."',surname='".$surname."',Spec='".$Spec."',address='".$address."',gender='".$gender."',téléphone='".$téléphone."' ,email='".$email."' where id='$id'";
				...
				if(move_uploaded_file($tmpname, $image_path)){ //MSK-000143-U-7
					$sql = "update doctor set  cin='" . $cin . "', fname='" . $fname . "',surname='" . $surname . "',Spec='".$Spec."',address='" . $address . "',gender='" . $gender . "',téléphone='" . $téléphone . "' ,email='" . $email . "',image_name='" . $image_path . "' where id='$id'";
	...
	header("Location: view/all_doctor.php?do=alert_from_update&msg=$msg&page=$c_page");//MSK-000143-U-23		
