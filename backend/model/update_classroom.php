...
if(isset($_GET["do"])&&($_GET["do"]=="update_classroom")){
	...
	$sql="SELECT * FROM classroom where name='$name'";	
	...
	if($name == $name1){
		if($id == $id1){
			if($admission_fee == $admission_fee1 && $hall_charge == $hall_charge1){
				...
				$sql1 = "update classroom set admission_fee='".$admission_fee."',hall_charge='".$hall_charge."' where id='$id'";
			...
			$msg+=4;
			//MSK-000143-U-2 The classroom is duplicated
		}
	
	}else{//MSK-000143-U-3
				
		$sql1 = "update classroom set name='".$name."',admission_fee='".$admission_fee."',hall_charge='".$hall_charge."' where id='$id'";
